-- BigQuery DDL for production deployment (GCP)
-- Replace {project} and {dataset} before running.

CREATE SCHEMA IF NOT EXISTS `{project}.{dataset}`;

CREATE TABLE IF NOT EXISTS `{project}.{dataset}.track_features` (
...
CREATE TABLE IF NOT EXISTS `{project}.{dataset}.promotion_features` (
  track_id STRING NOT NULL,
  promoted INT64,
  T INT64,
...
CREATE TABLE IF NOT EXISTS `{project}.{dataset}.engagement_features` (
  track_id STRING NOT NULL,
  streams INT64,
  likes INT64,
  saves INT64,
